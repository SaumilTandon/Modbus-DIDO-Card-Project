...
uint16_t adcValue[4] = {0};   // store PA0–PA3 values
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  // Update Live Expressions
	        ...
	        // Relay status check (store coil states for Live Expressions)
...
{
    MX_ADC_Init();
    HAL_ADCEx_Calibration_Start(&hadc);  // calibration step
}
...
void Modbus_Handle(uint8_t *frame, uint16_t size)
{
    ...
    if (func == 0x01) { // Read Coils
        ...
        for (int i=0; i<qty; i++) {
            uint16_t val = 0;
            switch (start+i) {
                case 0: val = My_ADC_Read(ADC_CHANNEL_3); break; // AIN1 (PA3)
                case 1: val = My_ADC_Read(ADC_CHANNEL_2); break; // AIN2 (PA2)
                case 2: val = My_ADC_Read(ADC_CHANNEL_1); break; // AIN3 (PA1)
                case 3: val = My_ADC_Read(ADC_CHANNEL_0); break; // AIN4 (PA0)
