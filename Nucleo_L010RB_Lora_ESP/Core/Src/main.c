...
uint8_t sx1262_get_status(void)
{
    ...
    HAL_GPIO_WritePin(SX1262_NSS_GPIO_Port,
                      SX1262_NSS_Pin,
                      GPIO_PIN_RESET);
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  uart_print("\r\n");
  uart_print("===== SX1262 TEST START =====\r\n");

  /* Reset SX1262 */
  HAL_GPIO_WritePin(SX1262_RESET_GPIO_Port,
                    SX1262_RESET_Pin,
                    GPIO_PIN_RESET);
  ...
  if(HAL_GPIO_ReadPin(SX1262_BUSY_GPIO_Port,
                      SX1262_BUSY_Pin))
  {
      uart_print("BUSY = HIGH\r\n");
  }
  else
  {
      uart_print("BUSY = LOW\r\n");
	  ...
	  uint8_t ch;
	      ...
	      if(HAL_UART_Receive(&hlpuart1, &ch, 1, 100) == HAL_OK)
	      {
	          if(ch == '\n')
	          {
	              ...
	              if(strcmp(rx_buffer, "PING\r") == 0)
	              {
	                  uart_print("PONG\r\n");
	              }

	              else if(strcmp(rx_buffer, "GET_STATUS\r") == 0)
	              {
	                  uart_print("SX1262_OK\r\n");
	              }

	              else if(strcmp(rx_buffer, "GET_CONFIG\r") == 0)
	              {
	                  uart_print("SF=9\r\n");
	                  uart_print("BW=125000\r\n");
	                  uart_print("FREQ=865062500\r\n");
	                  uart_print("POWER=22\r\n");
	              }

	              else
	              {
	                  uart_print("UNKNOWN_COMMAND\r\n");
	              }
